...
include('nomesgrupos.php');
   ...
   if ($mesatual == 1){ $mesatualtexto = 'JANEIRO'; };
   if ($mesatual == 2){ $mesatualtexto = 'FEVEREIRO'; };
   if ($mesatual == 3){ $mesatualtexto = 'MARÇO'; };
   if ($mesatual == 4){ $mesatualtexto = 'ABRIL'; };
   if ($mesatual == 5){ $mesatualtexto = 'MAIO'; };
   if ($mesatual == 6){ $mesatualtexto = 'JUNHO'; };
   if ($mesatual == 7){ $mesatualtexto = 'JULHO'; };
   if ($mesatual == 8){ $mesatualtexto = 'AGOSTO'; };
   if ($mesatual == 9){ $mesatualtexto = 'SETEMBRO'; };
   if ($mesatual == 10){ $mesatualtexto = 'OUTUBRO'; };
   if ($mesatual == 11){ $mesatualtexto = 'NOVEMBRO'; };
   if ($mesatual == 12){ $mesatualtexto = 'DEZEMBRO'; };

   $result = mysql_query("SELECT * FROM clientes");
   $num_rows = mysql_num_rows($result);
   $query = sprintf("SELECT * FROM clientes WHERE mes = '".$mesatualtexto."' and dia = '".$diaatual."' ORDER BY dia");
   ...
   $local_cookie = $_COOKIE['local'];
	mysql_query("insert into log (datahora, user, log1, log2, tipo, local)values(now(),'".$login_cookie."',' ','".$linha['dia']." / ".$linha['mes']."','Viu Aniversariantes - Dia: ','".$local."')");
...
<head>
    ...
    <title>:: Aniversariantes de Hoje ::</title>
...
?>
        <div id="page-wrapper">
            <div class="row">
                <div class="col-lg-12">
                    <h1 class="page-header">Aniversariantes de Hoje: <?=$linha['dia']?>/<?=$linha['mes']?></h1>
                </div>
                <!-- /.col-lg-12 -->
            </div>
            <!-- /.row -->
